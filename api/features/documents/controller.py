...
from api.features.documents.entities.document import DocumentType, DocumentStatus
...
class DocumentController:
    ...
    async def upload_document(
        ...
        await self.document_service.upload_to_storage(storage_path, file_content, file.content_type)
        download_url = await self.document_service.get_download_url(storage_path)
        
        # Queue processing task
        task = process_document.delay(doc_id=str(document.id), checksum=checksum)
    ...
    def _determine_document_type(self, filename: str, content_type: Optional[str]) -> DocumentType:
        """Determine document type from filename and content type."""
        filename_lower = filename.lower()
        
        # Map based on filename patterns for financial documents
        if 'transcript' in filename_lower  in filename_lower:
            return DocumentType.TRANSCRIPT
        elif 'earnings_release' in filename_lower in filename_lower:
            return DocumentType.EARNINGS_RELEASE
        elif 'slide' in filename_lower or 'presentation' in filename_lower or 'deck' in filename_lower:
            return DocumentType.SLIDE_DECK
        elif 'press' in filename_lower or 'announcement' in filename_lower or 'news' in filename_lower:
