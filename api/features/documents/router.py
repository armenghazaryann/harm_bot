...
from api.features.documents.exceptions import DocumentNotFoundError, DocumentValidationError
...
async def health_check():
    ...
    return ResponseModel.success(
        data=HealthCheckResponse(
            status="healthy",
            dependencies={"storage": "ok", "database": "ok"}
        ),
...
async def upload_document(
    ...
    controller: DocumentController = Depends(Provide[DependencyContainer.document_controller]),
    db_session = Depends(Provide[DependencyContainer.db_session])
):
    """Upload a document for processing."""
    try:
        result = await controller.upload_document(file, db_session)
        return ResponseModel.success(
            data=result,
...
async def get_document_status(
    ...
    try:
        result = await controller.get_document_status(doc_id, db_session)
        return ResponseModel.success(
            data=result,
...
async def list_documents(
    ...
    try:
        ...
        result = await controller.list_documents(pagination, status, db_session)
        return ResponseModel.success(
            data=result,
