...
class QueryRequest(BaseDTO):
    ...
    query: str = Field(
        ..., min_length=1, max_length=1000, description="The search query"
    )
    top_k: int = Field(
...
class AnswerRequest(BaseDTO):
    ...
    max_tokens: int = Field(
        default=500, ge=50, le=2000, description="Maximum response tokens"
    )
...
class AnswerResponse(BaseDTO):
    ...
    verification_report: Dict[str, Any] = Field(
        ...
        description="Self-RAG verification details (claims, labels, evidence)",
    )
    retrieval_diagnostics: Dict[str, Any] = Field(
        default_factory=dict,
        description="Diagnostics for retrieval, grading, and rewrites",
    )
...
class SuggestionRequest(BaseDTO):
    ...
    prefix: str = Field(
        default="", max_length=100, description="Query prefix for suggestions"
    )
    limit: int = Field(
        default=10, ge=1, le=50, description="Maximum number of suggestions"
    )
    context: Optional[str] = Field(
        default=None, description="Context for better suggestions"
    )
...
class SemanticSearchRequest(BaseDTO):
    ...
    document_filters: Optional[List[UUID]] = Field(
...
class KeywordSearchRequest(BaseDTO):
    ...
    boost_exact_match: bool = Field(
        default=True, description="Boost exact phrase matches"
    )
...
class HybridSearchRequest(BaseDTO):
    ...
    def validate_weights(cls, v, values):
        if "semantic_weight" in values:
            ...
            if (
                abs(semantic_weight + v - 1.0) > 0.001
