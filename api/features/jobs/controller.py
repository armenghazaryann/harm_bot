...
class JobController:
    ...
    async def get_job_status(
        ...
        try:
            ...
            return ResponseModel.success(
                data=response_data, message="Job status retrieved successfully"
            )
    ...
    async def list_jobs(
        ...
        try:
            ...
            return ResponseModel.success(
                data=response_data, message="Jobs retrieved successfully"
            )
    ...
    async def run_evaluation(
        self, request: EvalRunRequest, db_session: AsyncSession
    ) -> ResponseModel[EvalRunResponse]:
        """Run an evaluation suite."""
        # MVP: Evaluation workflows are disabled to keep the system lean.
        response_data = EvalRunResponse(
            run_id=None,
            status="disabled",
            message="Evaluation runs are disabled in MVP",
            estimated_duration_minutes=0,
        )
        return ResponseModel.success(
            data=response_data, message="Evaluation is disabled in MVP"
        )

    async def cancel_job(
        self, job_id: UUID, cancel_request: JobCancelRequest, db_session: AsyncSession
    ) -> ResponseModel[str]:
        """Cancel a running job."""
        try:
            success = await self.job_service.cancel_job(
                job_id=job_id, reason=cancel_request.reason, db_session=db_session
            )
    ...
    async def retry_job(
        self, job_id: UUID, retry_request: JobRetryRequest, db_session: AsyncSession
    ) -> ResponseModel[JobStatusResponse]:
        """Retry a failed job."""
        try:
            ...
            return ResponseModel.success(
                data=response_data, message="Job retried successfully"
            )
    ...
    async def get_job_stats(
        ...
        try:
            ...
            return ResponseModel.success(
                data=response_data, message="Job statistics retrieved successfully"
            )
