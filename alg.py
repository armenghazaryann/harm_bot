...
def find_candidate_labels(text, max_words=7):
    ...
    for m in re.finditer(r":", text):
        ...
        name_like = sum(
            1 for w in words if re.match(r"^[A-Z0-9\(\)\-][A-Za-z0-9\.\-&()]*$", w)
        )
...
def segment_by_labels(text, labels):
    ...
    pattern = re.compile(
        r"(?:" + "|".join(re.escape(lbl) for lbl in labels_sorted) + r")\s*:",
        flags=re.M,
    )
    dialogues = []
    last_pos = 0
    ...
    for m in pattern.finditer(text):
        ...
        if last_label is not None:
            ...
            dialogues.append(
                {"speaker": last_label.strip(), "speech": collapse_whitespace(speech)}
            )
    ...
    if last_label is not None and last_pos < len(text):
        dialogues.append(
            {
                "speaker": last_label.strip(),
                "speech": collapse_whitespace(text[last_pos:]),
            }
        )
    return dialogues
...
def repair_dialogues(dialogues):
    ...
    for d in dialogues:
        if speaker_looks_like_sentence(d["speaker"]):
            ...
            if repaired:
                repaired[-1]["speech"] = collapse_whitespace(
                    repaired[-1]["speech"] + " " + d["speaker"] + " " + d["speech"]
                )
                merged += 1
            else:
                ...
                repaired.append(
                    {
                        "speaker": "Unknown",
                        "speech": collapse_whitespace(d["speaker"] + " " + d["speech"]),
                    }
                )
                merged += 1
        else:
...
def main(pdf_path, out_json, out_json_repaired=None):
                    ...
                    dialogues.append(
                        {
                            ...
                            "speech": collapse_whitespace(" ".join(current_speech)),
                        }
                    )
    ...
    if out_json_repaired:
        ...
        print(
            f"Wrote repaired {len(repaired)} pairs to {out_json_repaired}  (merged {merged} suspicious entries)"
        )
...
if __name__ == "__main__":
    ...
    out = (
        args.out or os.path.splitext(os.path.basename(pdf_path))[0] + "_dialogues.json"
    )
